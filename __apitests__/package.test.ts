    ...
    describe('POST /package', () => {
        ...
        it('should return 400 for missing version', async () => {
            jest.mock('../src/helper', () => ({
                extractPackageJsonInfo: jest.fn().mockImplementation((content) => {
                    return { name: 'express', version: '', repositoryUrl: 'https://github.com/expressjs/express' }; // Mocked response with missing version
                }),
            }));
        ...
        it('should return 201 for valid package data', async () => {
            ...
            expect(response.body).toEqual({ error: 'Failed to fetch and process GitHub repository.' });
        });
    });
    ...
    describe('GET /package/:id', () => {
        ...
        it('should return 404 if package not found', async () => {
            jest.mock('../src/packages/packagedb', () => ({
                getPackageByID: jest.fn().mockImplementationOnce((id: number) => {
                    id = 999;
                })
            }));
            const response = await request(app).get('/package/999');
        ...
        it('should return 400 if missing field', async () => {
            const response = await request(app).get('/package/null');
